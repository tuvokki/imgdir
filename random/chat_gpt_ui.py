...
class ToolTip:
    ...
    def show_tooltip(self):
        ...
        label = tk.Label(self.tooltip_window, text=self.text, background="black", foreground="white", relief="solid",
                         borderwidth=1)
        label.pack()
...
def add_message(role="user", content=""):
    ...
    message["content_widget"].bind("<KeyRelease>",
                                   lambda event, content_widget=message["content_widget"]: update_content_height(event,
                                                                                                                 content_widget))
    update_content_height(None, message["content_widget"])
...
def prompt_paste_from_clipboard(event, entry):
    ...
    if previous_focused_widget != entry:
        ...
        if messagebox.askyesno("Paste from Clipboard",
                               f"Do you want to paste the following content from the clipboard?\n\n{clipboard_content}"):
...
def show_popup():
    ...
    dark_mode_checkbutton = ttk.Checkbutton(popup_frame, text="Dark mode", variable=dark_mode_var,
...
ttk.OptionMenu(main_frame, model_var, "gpt-3.5-turbo", "gpt-3.5-turbo", "gpt-4", "gpt-3.5-turbo-0301",
               "gpt-4-0314").grid(row=0, column=7, sticky="nw")
...
submit_button = ttk.Button(main_frame, textvariable=submit_button_text,
                           command=send_request)  # Use textvariable instead of text
...
# Add a separator
ttk.Separator(configuration_frame, orient='horizontal').grid(row=config_row + 1, column=0, columnspan=10, sticky="we",
                                                             pady=3)
